...
import { ArrowLeft, Target, TrendingUp, Calendar, Upload, FileCheck } from 'lucide-react'
...
interface BallDashboardClientProps {
  data: BallData
}

export default function BallDashboardClient({ data }: BallDashboardClientProps) {
        ...
        <div className="max-w-4xl mx-auto px-4 py-4">
          ...
          <div className="flex items-start gap-3">
            <div className="w-12 h-12 rounded-full bg-gradient-to-br from-barrels-gold to-barrels-gold-light flex items-center justify-center flex-shrink-0">
              <Target className="w-6 h-6 text-barrels-black" strokeWidth={2.5} />
            </div>
            <div>
              <p className="text-xs text-barrels-muted uppercase tracking-wide">4B System</p>
              <h1 className="text-xl font-semibold text-white">Ball</h1>
              <p className="text-sm text-barrels-muted mt-1">
                Your ball contact metrics and HitTrax data
              </p>
            </div>
          </div>
        </div>
      ...
      <main className="max-w-4xl mx-auto px-4 py-6 space-y-6">
        ...
        >
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {/* Ball Score */}
            <div>
              <p className="text-xs text-barrels-muted uppercase tracking-wide mb-1">Ball Score</p>
              <div className="flex items-baseline gap-2">
                <span className="text-3xl font-bold text-barrels-gold">{data.currentBallScore}</span>
            ...
            {/* Exit Velo */}
            <div>
              <p className="text-xs text-barrels-muted uppercase tracking-wide mb-1">Exit Velo</p>
              <div className="flex items-baseline gap-2">
                <span className="text-3xl font-bold text-white">{data.avgExitVelo}</span>
            ...
            {/* Launch Angle */}
            <div>
              <p className="text-xs text-barrels-muted uppercase tracking-wide mb-1">Launch Angle</p>
              <div className="flex items-baseline gap-2">
                <span className="text-3xl font-bold text-white">{data.avgLaunchAngle}</span>
            ...
            {/* Hard Hit % */}
            <div>
              <p className="text-xs text-barrels-muted uppercase tracking-wide mb-1">Hard Hit</p>
              <div className="flex items-baseline gap-2">
                <span className="text-3xl font-bold text-white">{data.hardHitPercent}</span>
                <span className="text-xs text-barrels-muted">%</span>
              </div>
            </div>
          </div>
        ...
        >
          <div className="flex items-center justify-between mb-3">
            <h2 className="text-base font-semibold text-white">HitTrax Import</h2>
            <Upload className="w-5 h-5 text-barrels-gold" />
          </div>
          <p className="text-sm text-barrels-muted mb-4">
            Upload your HitTrax CSV file to import ball contact metrics
          </p>
        ...
        >
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-base font-semibold text-white">Exit Velo Over Time</h2>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      <span className="text-xs text-white">{session.exitVelo} mph</span>
                      <span className="text-xs text-barrels-muted">•</span>
                      <span className="text-xs text-barrels-muted">{session.games} games</span>
                    </div>
                  </div>
                </div>

                <div className="text-right">
                  <p className="text-lg font-bold text-white">{session.ballScore}</p>
